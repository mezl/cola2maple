...
#define BMW_FORMAT	"\033[1;33;46m★%s \033[37;45m %s \033[m"	/* 收到的編碼 */
#define BMW_FORMAT2	"\033[1;33;41m☆%s \033[34;47m %s \033[m"	/* 送出的編碼 */

static int bmw_locus = 0;		/* 總共保存幾個編碼 (保留最近收到 BMW_LOCAL_MAX 個) */
static BMW bmw_lslot[BMW_LOCAL_MAX];	/* 保留收到的編碼 */

static int bmw_locat = 0;		/* 總共保存幾個編碼 (保留最近送出 BMW_LOCAL_MAX 個) */
static BMW bmw_lword[BMW_LOCAL_MAX];	/* 保留送出的編碼 */


int			/* 1:可以傳編碼給對方/與對方Talk  0:不能傳編碼給對方/與對方Talk */
can_override(up)
  UTMP *up;
{
  int ufo;

  if (up->userno == cuser.userno)	/* 不能傳編碼給自己(即使是分身) */
    return 0;
  ...
  /* itoc.010909.註解: 站長可以傳編碼給 鎖定/BBSNET... 的人，這樣好嗎？觀察中 */

  if (HAS_PERM(PERM_ALLACCT))	/* 站長、帳號管理員可以傳給任何人 */
    return 1;

  /* itoc.010909: 鎖定時不能被傳編碼 */
  ...
  if (!(up->ufo & UFO_CLOAK) || HAS_PERM(PERM_SEECLOAK))
  {
    ...
    if (ufo & UFO_PAGER)
      return is_ogood(up);		/* pager 關閉時只有被設好友能傳編碼 */
    else
      return !is_obad(up);		/* pager 打開時只要沒有被設壞人即可傳編碼 */
  }
  else
  {
    /* itoc.020321: 對方若隱形傳我編碼，我也可以被動回 */
    BMW *bmw;
    ...
    for (ufo = bmw_locus - 1; ufo >= 0; ufo--)
    {
      ...
      /* itoc.030718: 如果我重新上站了，那麼即使我上一次上站有丟對方編碼，對方也不可以回我
...
#ifdef BMW_DISPLAY		
static void
bmw_display(max)	/* itoc.010313: display 以前的編碼 */
  int max;
{
  int i;
  BMW *bmw;
...
static int bmw_request;	/* 1: 有新的編碼進來 */
  ...
  BMW *bmw;
{
  ...
  for (;;)
  {
    ...
    if (ch != Ctrl('R') && ch != Ctrl('T'))	/* 完成編碼輸入 */
      break;

    /* 有新的編碼進來，重繪編碼回顧，並將 bmw_pos 指向原來那個編碼 */
    if (bmw_request)
    {
      bmw_request = 0;
      bmw_pos = bmw_locus - 1;
  ...
  sprintf(fpath, "確定要送出《編碼》給 %s 嗎(Y/N)？[Y] ", up ? up->userid : "廣播");
  if (vans(fpath) != 'n')
  {
    int i;
    ...
    if (up)	/* 不是廣播 */
    {
      ...
      if (bmw_send(up, bmw))	/* 編碼送不出去，不寫入編碼紀錄檔 */
      {
	vmsg(MSG_USR_LEFT);
	if (bbsmode != M_BMW_REPLY)
	  restore_foot(slp, 2);
	return;
      ...
      /* lkchu.990103: 若是自己送出的編碼，存對方的 userid */
...
bmw_outz()
{
  ...
  /* itoc.030621: 由保留的送出編碼中，找出上次回這人的編碼是什麼 */
  ...
  move(b_lines - 1, 0);
  clrtoeol();
  prints(BMW_FORMAT2, bmw->userid, i >= 0 ? benz->msg : "【您最近沒有傳編碼給這位使用者】");
}
...
static UTMP *
bmw_lastslot(pos)	/* 找出最近一個可以回編碼的對象 */
  int pos;
{
  int max, times;
  UTMP *up, *uhead;
...
static UTMP *
bmw_firstslot(pos)	/* 找出最遠一個可以回編碼的對象 */
  int pos;
{
  int max, times;
  UTMP *up, *uhead;
  ...
  int key;
{
  ...
  if (key == Ctrl('R'))
    bmw_up = bmw_lastslot(pos == 0 ? max : pos - 1);	/* 由目前所在 pos 往下找一個可以回編碼的對象 */
  else /* if (key == Ctrl('T')) */
    bmw_up = bmw_firstslot(pos == max ? 0 : pos + 1);	/* 由目前所在 pos 往上找一個可以回編碼的對象 */
...
bmw_rqst()
{
  ...
  if (j)
  {
    ...
    /* Thor.980827: 為了防止列印一半(more)時編碼而後列印超過範圍踢人, 故存下游標位置 */
    cursor_save(); 

    sprintf(buf, BMW_FORMAT, mptr->userid, mptr->msg);
    outz(buf);

    /* Thor.980827: 為了防止列印一半(more)時編碼而後列印超過範圍踢人, 故還原游標位置 */
    cursor_restore();
  ...
  BMW *bmw;
{
  ...
  if (bmw->sender == cuser.userno)	/* 送出的編碼 */
  {
...
static int
bmw_rangedel(xo)	/* itoc.001126: 新增編碼區段刪除 */
  XO *xo;
{
  ...
  char *fpath;
{
  ...
  /* itoc.020126.註解: 可以直接拿 FN_AMW 來儲存即可，
     可是如果用 FN_BMW 重做一次的話，可以讓使用者在 t_bmw() 中自由 d 掉不要的編碼 */
...
bmw_log()
{
  ...
  if (!stat(fpath, &st) && S_ISREG(st.st_mode))
  {
    ...
    if ((cuser.ufo & UFO_NWLOG) || !st.st_size)	/* itoc.000512: 不儲存編碼記錄 */
    ...
    else
    {
      more(buf, (char *) -1);
#ifdef RETAIN_BMW
      op = vans("本次上站編碼處理 (M)移至備忘錄 (R)保留 (C)清除 (S)存證？[R] ");
#else
      op = vans("本次上站編碼處理 (M)移至備忘錄 (R)保留 (C)清除？[R] ");
#endif
    }
...
#ifdef RETAIN_BMW
    case 's':
      if (vans("存證是把編碼轉寄給站長以檢舉其他使用者，您確定要存證嗎(Y/N)？[N] ") == 'y')
