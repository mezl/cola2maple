...
#define NECKER_PAL	"[←]離開 [a]新增 [c]修改 [d]刪除 [m]寄信 [w]編碼 [s]整理 [→]查詢 [h]說明\n" \
			COLOR3 "  編號    代 號         友       誼%*s                                           \033[m"

#define NECKER_ALOHA	"[←]離開 [a]新增 [d]刪除 [D]區段刪除 [m]寄信 [w]編碼 [s]重整 [f]引入 [h]說明\n" \
			COLOR3 "  編號   上 站 通 知 名 單%*s                                                    \033[m"

#define NECKER_VOTE	"[←]離開 [R]結果 [^P]舉行 [E]修改 [V]預覽 [^Q]改期 [o]名單 [h]說明\n" \
			COLOR3 "  編號      開票日   主辦人       投  票  宗  旨%*s                              \033[m"

#define NECKER_BMW	"[←]離開 [d]刪除 [D]區段刪除 [m]寄信 [M]儲存 [w]編碼 [s]更新 [→]查詢 [h]說明\n" \
...
#define FEETER_ULIST	\
COLOR1 " 網友列表 " COLOR2 " (f)好友 (t)聊天 (q)查詢 (ad)交友 (m)寄信 (w)編碼 (s)更新 (TAB)切換 "
...
#define FEETER_BMW	\
COLOR1 " 編碼回顧 " COLOR2 " (d)刪除 (D)區段刪除 (m)寄信 (w)編碼 (^R)回訊 (^Q)查詢 (s)更新      "
